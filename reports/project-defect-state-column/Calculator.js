Ext.define('Calculator', {
    ...
    prepareChartData: function(store) {
        let data = this._groupData(store),
        ...
        if (!this.stackField) {
            if(this.calculationType === 'count') {
                ...
                seriesData = _.map(data, function(value, key) {
                    let valueTotal = _.reduce(value, function(total, r) {
                        let valueField = this._getValueFieldForCalculationType();
            ...
            let stackField = store.model.getField(this.stackField),
                stackValues;
            ...
            if (this.stackValues) {
                ...
                let values = _.invoke(store.getRange(), 'get', this.stackField);
                if (this.stackField === 'Iteration' || this.stackField === 'Release') {
                    values = _.sortBy(values, function(timebox) {
                        let dateValue = timebox && (timebox.StartDate || timebox.ReleaseStartDate || null);
            ...
            _.each(categories, function(category) {
                let group = data[category];
                let recordsByStackValue = _.groupBy(group, function(record) {
                ...
                _.each(stackValues, function(stackValue) {
                    ...
                    let records = recordsByStackValue[stackValue];
                    if(this.calculationType === 'count') {
                        series[stackValue].push((records && records.length) || 0);
                    } else {
                        let valueTotal = _.reduce(records, function(total, r) {
                            let valueField = this._getValueFieldForCalculationType();
                            return total + r.get(valueField);
                        }, 0, this);
    ...
    _groupData: function(store) {
        let field = store.model.getField(this.field),
            fieldType = field.getType(),
            groups = {};
        if (fieldType === 'collection') {
            _.each(store.getRange(), function(record) {
                let value = record.get(this.field),
            ...
            if (fieldType === 'date') {
                let dates = _.sortBy(_.compact(_.map(store.getRange(), function(record) {
                    return record.get(this.field); 
                }, this)));
                let datesNoGaps = this._getDateRange(dates[0], dates[dates.length-1]);
                let allGroups = {};
                if (groups['-- No Entry --']) {
                    allGroups['-- No Entry --'] = groups['-- No Entry --'];
                }
                groups = _.reduce(datesNoGaps, function(accum, val) {
                    let group = this._getDisplayValue(field, moment(val).toDate());
    ...
    _getDisplayValueForField: function(record, fieldName) {
        let field = record.getField(fieldName),
